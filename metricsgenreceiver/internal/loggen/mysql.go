...
func mysqlDebugLogs(rng *rand.Rand) []MessageTemplate {
	...
	return []MessageTemplate{
		...
		},
		{
			...
			Attrs:        map[string]ArgGenerator{"db.system": Static("mysql"), "db.operation.name": Static("ANALYZE")},
		},
		{
			Severity:     plog.SeverityNumberDebug,
			...
			AttrFromArg:  map[string]int{"db.name": 3, "db.sql.table": 4},
			Attrs:        map[string]ArgGenerator{"db.system": Static("mysql"), "db.operation.name": Static("SELECT")},
		},
	}
}
